...
class PitchComparator {
    ...
    static PitchDifference retrieveNote(float pitch , int activity) {
        Tuning tuning = MainActivity.getCurrentTuning();
        Note[] notes = tuning.getNotes();
        if(activity == 2){
            TypeOfScalesOrArpeggios scaleOrArpeggio = Scales.getCurrentScaleOrArpeggio();
             notes = scaleOrArpeggio.getNotes();
        }
