...
class CanvasPainter {
    ...
    private boolean[] correctNotesArray;
    private Note[] notesInScaleArray;
    ...
    void on(Canvas canvas) {
        ...
        if (pitchDifference != null && Math.abs(getNearestDeviation()) <= MAX_DEVIATION) {
            ...
            if(test)
                findCorrectNotes(notesInScaleArray, correctNotesArray, pitchDifference);
    ...
    private void drawFrequency(){
        float y = canvas.getHeight() * 0.35f;
        TextPaint paint = new TextPaint(ANTI_ALIAS_FLAG);
        paint.setColor(textColor);
        int size = (int) (textPaint.getTextSize() / 1.25);
        paint.setTextSize(size);
        if(frequency < 10)
            canvas.drawText(frequency + " Hz",x - 125, y, paint);
        else if(frequency< 100)
            canvas.drawText(frequency + " Hz",x - 150, y, paint);
        else if(frequency < 1000)
            canvas.drawText(frequency + " Hz",x - 175, y, paint);
        else if(frequency< 10000)
            canvas.drawText(frequency + " Hz",x - 200, y, paint);
    }
    ...
    private void drawSymbols(float spaceWidth) {
        ...
        int symbolsTextSize = context.getResources().getDimensionPixelSize(R.dimen.symbolsTextSize) + 50;
        symbolPaint.setTextSize(symbolsTextSize);
        symbolPaint.setColor(textColor);

        float yPos = canvas.getHeight() / 4F;
        canvas.drawText(sharp,
                x + NUMBER_OF_MARKS_PER_SIDE * spaceWidth - symbolPaint.measureText(sharp) / 2F - 80,
                yPos, symbolPaint);

        canvas.drawText(flat,
                x - NUMBER_OF_MARKS_PER_SIDE * spaceWidth - symbolPaint.measureText(flat) / 2F + 80,
                yPos,
                symbolPaint);
    }
    ...
    public void findCorrectNotes(Note[] notesArray, boolean[] correctNotesArray, PitchDifference pitchDifference) {
        for (int i = 0; i < notesArray.length; i++) {
            if (pitchDifference != null && notesArray[i] == pitchDifference.closest && Math.abs(getNearestDeviation()) < 20) {
                correctNotesArray[i] = true;
            }
        }
    }
    public void setCorrectNotesArray(boolean[] correctArray){
        correctNotesArray = correctArray;
        }
    public void setNotesInScaleArray(Note[] notesInScale){
        notesInScaleArray = notesInScale;
    }
    public void setTest(boolean isTesting)
    {
        test = isTesting;
    }
    public boolean[] getCorrectNotesArray(){
            return correctNotesArray;
        }
    }
